...
// The forward target is a bare URL. Anyone who learns it can write fake traffic
// into the record, and a signature verified upstream means nothing if the pipe
// itself is open. Send a shared token so the receiver can refuse everything else.
//
// The header name is configurable because the receiver decides it, not us. An
// n8n Header Auth credential carries one fixed header name, and an existing one
// may already use something else (x-api-key is n8n's own default). Point
// RIPPILY_FORWARD_HEADER at whatever the credential expects rather than forcing
// a second credential into existence.
const DEFAULT_FORWARD_HEADER = 'X-Bee-Cairn-Token';

function forwardAuthHeaders() {
  const token = process.env.RIPPILY_FORWARD_TOKEN;
  if (!token) return {};
  const header = (process.env.RIPPILY_FORWARD_HEADER || '').trim() || DEFAULT_FORWARD_HEADER;
  return { [header]: token };
}
